...
import { OnboardingErrorBoundary } from "@/app/(auth)/onboarding/components/error-boundary";
import { OnboardingStepper, getStepsForAccountType } from "@/app/(auth)/onboarding/components/onboarding-stepper";
import { AccountTypeStep } from "@/app/(auth)/onboarding/components/account-type-step";
import { PersonalWorkspaceStep } from "@/app/(auth)/onboarding/components/personal-workspace-step";
import { OrgDetailsStep } from "@/app/(auth)/onboarding/components/org-details-step";
import { OrgWorkspaceStep } from "@/app/(auth)/onboarding/components/org-workspace-step";
import { CompletionStep } from "@/app/(auth)/onboarding/components/completion-step";
...
/**
 * BYOB Onboarding Page
 * 
 * Renders the same onboarding content as the Cloud onboarding page,
 * but redirects to /{orgSlug}/workspaces/:id on completion instead of /workspaces/:id.
 * 
 * sessionMiddleware routes all API calls (CreateOrg, CreateWorkspace) to the
 * customer's Appwrite automatically — no special handling needed.
 */
function BYOBOnboardingContent() {
    ...
    useEffect(() => {
        if (!isUserLoading && !user) {
            router.replace(`/${orgSlug}/sign-in`);
        }
    }, [isUserLoading, user, router, orgSlug]);
...
export default function BYOBOnboardingPage() {
    return (
        <OnboardingErrorBoundary>
            <BYOBOnboardingContent />
        </OnboardingErrorBoundary>
    );
}
