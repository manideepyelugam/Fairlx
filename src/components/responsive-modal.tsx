...
export const ResponsiveModal = ({ children, open, onOpenChange, showCloseButton = true }: ResponsiveModalProps) => {
  ...
  if (variant === "desktop") {
    return (
      <Dialog open={open} onOpenChange={handleChange}>
        <DialogContent className={`w-full sm:max-w-lg p-0 border-none overflow-y-auto hide-scrollbar max-h-[85vh] ${!showCloseButton ? '[&>button]:hidden' : ''}`}>
          <VisuallyHidden>
