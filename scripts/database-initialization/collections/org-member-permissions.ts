...
const COLLECTION_ID = process.env.NEXT_PUBLIC_APPWRITE_ORG_MEMBER_PERMISSIONS_ID || 'org_member_permissions';
const COLLECTION_NAME = 'Org Member Permissions';

export async function setupOrgMemberPermissions(databases: Databases, databaseId: string): Promise<void> {
    ...
    await ensureIndex(databases, databaseId, COLLECTION_ID, 'organizationId_memberId_idx', IndexType.Unique, ['organizationId', 'memberId']);
}
