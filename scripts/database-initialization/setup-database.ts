...
function getUserRequiredVars(): EnvVarEntry[] {
    return [
        // Razorpay
        {
            ...
            description: 'Razorpay API Key ID → https://dashboard.razorpay.com/app/keys',
        },
        {
            key: 'RAZORPAY_KEY_SECRET',
            value: process.env.RAZORPAY_KEY_SECRET || '',
            description: 'Razorpay API Key Secret → https://dashboard.razorpay.com/app/keys',
        },
        {
            key: 'RAZORPAY_WEBHOOK_SECRET',
            value: process.env.RAZORPAY_WEBHOOK_SECRET || '',
            description: 'Razorpay Webhook Secret → Settings > Webhooks in Razorpay dashboard',
        },
        {
            key: 'RAZORPAY_BASE_PLAN_ID',
            value: process.env.RAZORPAY_BASE_PLAN_ID || '',
            description: 'Razorpay Plan ID (optional) → Create a plan in Razorpay dashboard',
        },
        {
...
async function main() {
    ...
    for (const { name, setup } of bucketSetups) {
        try {
            await setup(storage);
        } catch (err) {
            const message = err instanceof Error ? err.message : String(err);
            console.error(`\x1b[31m❌ Fatal error in bucket "${name}": ${message}\x1b[0m`);
            console.error(`\x1b[33m   Continuing with next bucket...\x1b[0m`);
        }
    }
